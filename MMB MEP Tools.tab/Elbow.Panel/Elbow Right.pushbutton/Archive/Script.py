...
# Naviate LT convention
if abs(forward.Z) > 0.99:  # vertical duct/pipe/conduit
    right_vector = XYZ(-1, 0, 0)  # opposite of BasisX
else:
    right_vector = XYZ.BasisZ.CrossProduct(horizontal_forward).Normalize()

# -------------------------------------------------
# SEGMENT LENGTH (strict thresholds)
# -------------------------------------------------
segment_length = 0.0
...
try:
    if isinstance(element, Duct):
        ...
        logger.info("✅ Conduit segment created with matching size (right bend).")

    t.Commit()
    logger.info("✅ Elbow/Bend created with strict stub length rules.")
