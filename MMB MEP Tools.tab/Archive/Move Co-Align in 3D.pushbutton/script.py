...
def align_in_3d():
    ...
    with DB.Transaction(doc, "Align in 3D") as t:
        ...
        try:
            ...
            # ---------------------------------------------------------
            # Re-read connectors after rotation
            # ---------------------------------------------------------

            doc.Regenerate()

            dest_conns_after = get_connectors(dest_el)
            move_conns_after = get_connectors(move_el)

            dest_conn_after, move_conn_after = closest_connector_pair(
                dest_conns_after,
                move_conns_after
            )

            target_origin_after = dest_conn_after.Origin
            move_origin_after = move_conn_after.Origin

            # ---------------------------------------------------------
            # Move connector onto destination connector
            # ---------------------------------------------------------
            ...
            try:
                if not move_conn_after.IsConnected:
                    move_conn_after.ConnectTo(dest_conn_after)
            except Exception as ex:
                logger.warning(
                    "ConnectTo failed: {}".format(ex)
                )
