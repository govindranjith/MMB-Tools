...
if horizontal_forward.IsZeroLength():
    logger.error("Elbow UP requires horizontal duct or pipe.")
    script.exit()

# -------------------------------------------------
# UP VECTOR ✅ (GLOBAL Z AXIS)
# -------------------------------------------------
...
end_point = start_point + up_vector.Multiply(segment_length)
...
t = Transaction(doc, "Elbow Up Tool")
t.Start()
...
try:
    if is_duct:
        ...
        logger.info("✅ Elbow UP and new duct created.")
        ...
        doc.Create.NewElbowFitting(c1, c2)
        logger.info("✅ Elbow UP and new pipe created.")
