...
# -------------------------------------------------
# DETERMINE FORWARD DIRECTION (Must be horizontal)
# -------------------------------------------------
...
if horizontal_forward.IsZeroLength():
    logger.error("Elbow DOWN requires a horizontal duct or pipe.")
    script.exit()

# -------------------------------------------------
# DOWN VECTOR ✅ (NEGATIVE Z AXIS)
# -------------------------------------------------
down_vector = XYZ.BasisZ.Negate()

# -------------------------------------------------
# CREATE NEW SEGMENT + ELBOW
# -------------------------------------------------
start_point = available_connector.Origin
end_point = start_point + down_vector.Multiply(segment_length)
...
t = Transaction(doc, "Elbow Down Tool")
t.Start()
...
try:
    if is_duct:
        ...
        logger.info("✅ Elbow DOWN and new duct created.")
        ...
        doc.Create.NewElbowFitting(c1, c2)
        logger.info("✅ Elbow DOWN and new pipe created.")
